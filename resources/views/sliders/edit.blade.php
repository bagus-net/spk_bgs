        ...
        <div class="mb-3">
            <div class="float-end">
                <span>
                    ...
                    <input class="radio form-check-input me-3" type="radio" name="radioNoLabel" id="radio-normal" value="normal" aria-label="..." onclick="check()">
                </span>
                <span>
                    <label for="">Custom</label>
                    <input class="radio form-check-input" type="radio" name="radioNoLabel" id="radio-custom" value="custom" aria-label="..." onclick="check()">
                </span>
            </div>
        </div>
    </div>
...
<div class="mt-3">
    {{-- form --}}
    <form method="POST" action="{{url('slider-update')}}" id="form-update" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="title" id="title"/>
        <input type="hidden" name="id" id="id-slider"/>
...
<label for="" class="mt-3">Display</label>
<select name="" id="display-flag" class="form-control">
...
<script>
    ...
    $(document).ready(function(){
        console.log(slider);
        $("#radio-"+slider.type).prop("checked", true);
        check();

        editor.getDoc().setValue(slider.custom_script);
        $("#add-title").val(slider.title);
        $("#add-custom-title").val(slider.title);
        $("#add-active-date").val(slider.active_date);
        $("#add-below-title").val(slider.below_title);
        $("#add-desc").val(slider.desc);
        $("#add-target-url").val(slider.shop_url);
        $("#display-flag").val(slider.flag);
        $("#id-slider").val(slider.id);
        
        const objTextForm = JSON.parse(slider.json_text_form);
        console.log(objTextForm);
        if (objTextForm.title == "bold")
            $("#radio-bold-title").prop("checked", true);
        else 
            $("#radio-thin-title").prop("checked", true);
        
        if (objTextForm.below_title == "bold")    
            $("#radio-bold-belowtitle").prop("checked", true);
        else 
            $("#radio-thin-belowtitle").prop("checked", true);

        if (objTextForm.desc == "bold")    
            $("#radio-bold-desc").prop("checked", true);
        else 
            $("#radio-thin-desc").prop("checked", true);    

        if (slider.text_color == "dark"){
            $("#radio-dark").prop("checked", true);
            textColor = "dark";
        } else {
    ...
    $("#btn-save").on('click', function(){
        ...
        if (str) {
            ...
            const customScript  = editor.getDoc().getValue();
            const targetUrl     = $("#add-target-url").val();
            const display       = $("#display-flag option:selected").val();

            let title = "";
            let isAllowSubmit = false;
            if (type == "normal"){
                title = $("#add-title").val();
                // isAllowSubmit = belowTitle != "" && desc != "" && activeDate != "" && targetUrl != "" && title != "";
